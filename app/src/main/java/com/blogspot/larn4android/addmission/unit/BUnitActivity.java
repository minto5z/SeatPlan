...
public class BUnitActivity extends AppCompatActivity {
    ...
    private EditText etBunitRoll;
    private String RollB ="";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_bunit);


        etBunitRoll = (EditText)findViewById(R.id.etBunitRoll);
        Button btn = (Button)findViewById(R.id.btn_BunitRoll);
        btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                RollB = etBunitRoll.getText().toString();
                len = RollB.length();
                InputRoll = RollB;
                if(!RollB.equals("")&&len==5) {
                    try{
                        int roll = parse(RollB);
                        if(roll<40001||roll>67500 ){
                            Toast.makeText(getApplicationContext(),"Please Enter Correct Roll Number!",Toast.LENGTH_SHORT).show();
                        }
                        else processRoll(RollB);
                    }
                    catch (Exception e)
                    {
...
//        if(str==null)return  -1;
    ...
    public void processRoll(String RollNum) throws Exception
    {
        ...
        try {
            ...
            InputStream is=am.open("bunit2016.xls");
