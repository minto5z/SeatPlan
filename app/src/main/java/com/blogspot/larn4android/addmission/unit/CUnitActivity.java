...
public class CUnitActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etCunitRoll = (EditText)findViewById(R.id.etCunitRoll);
        Button btn = (Button)findViewById(R.id.btn_CunitRoll);
        btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                RollC = etCunitRoll.getText().toString();
                InputRoll = RollC;
                len = RollC.length();
                if(!RollC.equals("")&&len==5) {
                    try {
                        int roll = parse(RollC);
                        if(roll<70001||roll>80193){
                            Toast.makeText(getApplicationContext(),"Please Enter Correct Roll Number!",Toast.LENGTH_SHORT).show();
                        }
                        else processRoll(RollC);
                    } catch (Exception e) {
...
//        if(str==null)return  -1;
    ...
    public void processRoll(String RollNum) throws Exception
    {
        ...
        try {
            ...
            InputStream is=am.open("cunit2016.xls");
